...
class SUAAseg(models.Model):
    ...
    complete_row_afil = fields.Char(string='Registro Completo para Formato SUA Aseg.txt')
    ...
    def _compute_name(self):
        for record in self:
            record.name = "ALTA: %s %s %s" % (record.employee_id.name ,record.fecha_de_alta,record.salario_diario_integrado) 
...
# === Override ORM Methods sua.aseg for template of ASEG.txt===
    ...
    def get_complete_row_aseg(self):
        self.complete_row_afil=self.get_full_row_ASEG()
