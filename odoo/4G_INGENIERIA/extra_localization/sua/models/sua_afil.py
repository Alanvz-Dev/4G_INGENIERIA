...
class SUAAfil(models.Model):
    ...
    def _compute_name(self):
        for record in self:
            record.name = "DATOS AFILIATORIOS: %s %s %s %s" % (record.employee_id.name,record.registro_patronal_imss,record.clave_lugar_de_nacimiento,record.numero_de_seguridad_social) 

    @api.onchange('lugar_de_nacimiento')
    def _onchange_lugar_de_nacimiento(self):
        if self.lugar_de_nacimiento.id:
    ...
    def _compute_digito_verificador_de_seguridad_social(self):
        if self.numero_de_seguridad_social and len(self.numero_de_seguridad_social)==LONG11:
    ...
    def _compute_digito_verificador_de_registro_patronal(self):
        for item in self:
            if item.registro_patronal_imss and len(item.registro_patronal_imss)==LONG11:
                item.digito_verificador_registro_patronal=item.registro_patronal_imss[-1]    
    ...
    def _compute_lugar_de_nacimiento_formato_sua(self):
        if self.lugar_de_nacimiento_formato_sua:
...
# === Define Errors sua.aseg for template of ASEG.txt===
    ...
    def get_full_row_AFIL(self):
        return self.registro_patronal_imss[:-1]+self.digito_verificador_registro_patronal+self.numero_de_seguridad_social[:-1]+self.digito_verificador_numero_de_seguridad_social+\
            self.codigo_postal+self.fecha_de_nacimiento+self.fill_empty_or_incomplete(FILLSPACE,LONG25,REPLACELEFT,str(self.lugar_de_nacimiento.descripcion))+\
                self.fill_empty_or_incomplete(FILLZERO,LONG2,REPLACELEFT,str(self.lugar_de_nacimiento.id))+self.unidad_de_medicina_familiar+self.fill_empty_or_incomplete(FILLSPACE,LONG12,REPLACERIGHT,self.ocupacion)+self.sexo+\
                self.tipo_de_salario+' '
