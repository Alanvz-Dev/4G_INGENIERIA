...
class purchase_order(models.Model):
    ...
    cuenta_analitica=fields.Many2one('account.analytic.account','Cuenta Analitica / Proyecto')
    autorizacion_compras=fields.Boolean('Liberada por Compras')
    pendiente_envio=fields.Boolean('Pendiente Envio OC')
    ...
    def update_cuanta_analitica(self):
        if self.cuenta_analitica.id:
            ...
            raise UserError(_("No se a seleccionado cuenta analitica o proyecto"))

    @api.one #(cr, uid, ids, context)
    def button_confirm(self):
        res = super(purchase_order, self).button_confirm()
        #if self.partner_id.proveedor_invalido != False:
            #raise UserError(_("No se puede asignar este proveedor para una Orden de Compra."))
        return res
