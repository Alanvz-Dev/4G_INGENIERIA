    ...
    def _compute_total(self):
        ...
        if self.cdv_lines:
            for line in self.cdv_lines:
                if line.tipo=='costo_dpr':
                    costo_de_produccion=line.importe
                elif line.tipo=='inventario':
                    inventarios=line.importe
                print(line)
            self.total=costo_de_produccion+inventarios
            print(self.total)
        else:
            self.total=0.0
...
class CostoDeVentasLine(models.Model):
    ...
    tipo = fields.Selection(selection=[('mat_consu', 'Material Consumido'), ('costo_dpr', 'Costo de Producción'),('inventario', 'Inventario')])
