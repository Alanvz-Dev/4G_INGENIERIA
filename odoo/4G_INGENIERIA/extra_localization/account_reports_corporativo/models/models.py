...
class TrialBalanceReport(models.TransientModel):
    ...
    def dfs_list_edo_de_situacion_financiera(self,model,resultado_del_ejercio_anterior,data_for_report_as_data_frame):                    
        # list of dataframes
        return [model.compute_total_activo_circulante(data_for_report_as_data_frame),model.compute_total_pasivo(data_for_report_as_data_frame),model.compute_total_capital_contable(data_for_report_as_data_frame,resultado_del_ejercio_anterior)]
    ...
    def compute_total_activo_circulante(self,df):
        ACTIVO=['1-101','1-102','1-104','1-106','1-170','1-109','1-110','1-111']
    ...
    def compute_total_pasivo(self,df):
        PASIVO_A_CORTO_PLAZO=['2-001','2-003','2-006','2-007','2-008','2-009']
    ...
    def compute_total_capital_contable(self,df,resultado_de_ejercicio_anterior):
        CAPITAL_CONTABLE = ['3-001','3-002','3-003']
...
###ESTADO DE PERDIDAS Y GANANCIAS
    def compute_r1(self,df):
        ...
        print(df_ventas_netas)
        ventas_netas=df_ventas_netas['result_d_c_']._values[0]
        print(ventas_netas)
        df_costo_de_ventas=df_r1[df_r1['group_code_prefix']=='4-200']
        print(df_costo_de_ventas)
        costo_de_ventas=df_costo_de_ventas['result_d_c_']._values[0]
        new_row = {'name':'TOTAL INGRESOS (R1)', 'group_code_prefix':"SERVICIOS + OTROS INGRESOS = R1", 'result_d_c_':ventas_netas-costo_de_ventas}
    ...
    def compute_r2_r3(self,df,R1_result):
        R2=['5-101','5-102','5-201']
        df_r2 =self.generate_data_frame_for_acoount_group(R2,df)
        df_gastos_de_sueldos=df_r2[df_r2['group_code_prefix']=='5-101']
        gastos_de_sueldos=df_gastos_de_sueldos['result_d_c_']._values[0]
        
        df_gastos_de_administracion=df_r2[df_r2['group_code_prefix']=='5-102']
        gastos_de_administracion=df_gastos_de_administracion['result_d_c_']._values[0]
        
        df_gastos_de_venta=df_r2[df_r2['group_code_prefix']=='5-201']
        gastos_de_venta=df_gastos_de_venta['result_d_c_']._values[0]
        
        
        new_row = {'name':'TOTAL GASTOS (R2)', 'group_code_prefix':"Ʃ GASTOS = R2", 'result_d_c_':gastos_de_sueldos+gastos_de_administracion+gastos_de_venta}
        df_r2=df_r2.append(new_row,ignore_index=True)
        new_row = {'name':'Utilidad o Perdida del Ejercicio 2021 (R3)', 'group_code_prefix':"R1 - R2 = R3", 'result_d_c_':R1_result-(gastos_de_sueldos+gastos_de_administracion+gastos_de_venta)}
        df_r2=df_r2.append(new_row,ignore_index=True)        
        return (df_r2,R1_result-R1_result-(gastos_de_sueldos+gastos_de_administracion+gastos_de_venta))
