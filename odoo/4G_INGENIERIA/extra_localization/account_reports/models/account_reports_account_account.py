...
class account_reports_account_account(models.Model):
    _name = 'account_reports.account_account'
    ...
    cuenta = fields.Many2one('account.account') 
    ...
    def _onchange_cuenta(self):
        if self.cuenta.id:
            search_res = self.concepto_id.report_id.trial_balance_lines.filtered(lambda r: r.account_id.id == self.cuenta.id)
