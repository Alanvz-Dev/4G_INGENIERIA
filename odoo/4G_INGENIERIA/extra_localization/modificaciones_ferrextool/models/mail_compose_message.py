    ...
    def send_mail(self, auto_commit=False):
        ...
        for wizard in self:
            ...
            for res_ids in sliced_res_ids:
                ...
                if self.model in ['sale.order'] and self._module in ['purchase']:
                    if batch_mails.state in ['sent']:
                        self.env[self.model].browse(self.res_id).update({'fecha_rfq':fields.datetime.now()})                    
                    else:
                        self.env[self.model].browse(self.res_id).update({'fecha_rfq':fields.datetime.now()})
                        return {
                            ...
                            'warning': {
                                ...
                                'message': 'El correo pudo no haberse entregado correctamente a todos los destinatarios, verifique que las direcciones de correo sean correctas.'}
