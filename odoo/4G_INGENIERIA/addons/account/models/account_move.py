...
##$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    ...
    def remove_trash_move_lines(self):
            revisar=[]
            self.ensure_one()
            self.env.cr.execute("""select * from ( select aml.account_id,sum(aml.debit) "debit",sum(aml.credit) "credit" from account_move_line aml  where aml.move_id ="""+str(self.id)+""" group by account_id ) as tbl""")
            account_debits_credits = self.env.cr.dictfetchall()
            for item in account_debits_credits:
                if item['debit'] >0 and item['credit'] >0:
                    if item['debit']==item['credit']:                                            
                        self.env.cr.execute("""select aml.id from account_move_line aml where aml.move_id = """+str(self.id)+""" and account_id  in ( select account_id from ( select aml.account_id,sum(aml.debit) "debit",sum(aml.credit) "credit" from account_move_line aml  where aml.move_id ="""+str(self.id)+""" group by account_id ) as tbl where tbl.debit=tbl.credit )""")        
                        move_line_ids = self.env.cr.dictfetchall()
                        for item in move_line_ids:
                            line=self.env['account.move.line'].browse(item['id'])
                            line.remove_move_reconcile()
                            self.env.cr.execute("""DELETE FROM public.account_move_line WHERE id="""+str(line.id)+"""""")
                        print("Poliza Depurada\t",self.id)
                    else:
                        print("Revisar",'\t',self.id)  
                        revisar.append(self.id)                      
    ...
    def post(self):
        ...
        for move in self:
            if move.name == '/':           
                ...
                if new_name:
                    move.name = new_name                    
                    self.write({'contabilidad_electronica': True})
    ...
    def create_tax_cash_basis_entry(self, percentage_before_rec):
        ...
        with self.env.norecompute():
            ...
            for move in {self.debit_move_id.move_id, self.credit_move_id.move_id}:
                ...
                ###Lines
                for line in move.line_ids:
                    #account.move.line(408716, 408715, 408714, 408713, 408712)
        ...
        if newly_created_move:
            ...
            self.env.cr.commit()
            newly_created_move.remove_trash_move_lines()
