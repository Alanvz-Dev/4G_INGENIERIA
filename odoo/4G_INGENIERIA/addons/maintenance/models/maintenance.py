...
class MaintenanceEquipment(models.Model):
    ...
    maintenance_cost = fields.Float(compute='_compute_cost_of_maintenance_price', string="Costo de Mantenimiento",digits=(10,4))
    analytic_acc_name = fields.Many2one('account.analytic.account',
                           'Nombre / Cuenta Analítica')

    maintenance_open_count = fields.Float(compute='_compute_maintenance_count', string="Current Maintenance", store=True)
    ...
    @api.one
    def _compute_cost_of_maintenance_price(self):
        result = 0.0
        acc_ids=[]
        acc_ids.append(self.analytic_acc_name.id)
        if acc_ids[0]==False:
            self.maintenance_cost=result
            return

        analytic_movs= self.env['account.move.line'].search([('contabilidad_electronica', '=','True'),('analytic_account_id', 'in', acc_ids)])
        for item in analytic_movs:
...
# "Please change the decorator from @api.one to @api.multi, as @api.one doesn't allow return action item."
    @api.multi
    def cost_of_maintenance_movs(self):
        acc_ids=[]
        acc_ids.append(self.analytic_acc_name.id)
        if acc_ids[0]==False:
            return
        analytic_movs= self.env['account.move.line'].search([('contabilidad_electronica', '=','True'),('analytic_account_id', 'in',acc_ids)]).mapped('id')
        print(analytic_movs)
        return {
            'type': 'ir.actions.act_window',
            'name': 'Movimientos de : '+self.name,
            'view_type': 'form',
            'view_mode': 'tree,form',
            'res_model': 'account.move.line',            
            'domain': [('id', 'in', analytic_movs)],
            #'target': 'current'
        }
...
class MaintenanceRequest(models.Model):
    ...
    worked_hours = fields.Float('Horas Trabajadas')    
    employee_repair = fields.Many2one('hr.employee',string="Empleado que reparó")
    employee_recive = fields.Many2one('hr.employee',string="Empleado que recibió")
    ...
    def write(self, vals):
        ...
        if vals.get('owner_user_id') or vals.get('technician_user_id'):
            self._add_followers()
        if 'stage_id' in vals:
            self.filtered(lambda m: m.stage_id.done).write({'close_date': fields.Datetime.now()})
        return res
