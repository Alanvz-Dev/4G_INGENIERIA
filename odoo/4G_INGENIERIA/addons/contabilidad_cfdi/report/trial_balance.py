...
SELECT
    ...
    acc.name,
    CASE 
        WHEN acc.cuenta_tipo='D' OR ag.cuenta_tipo='D' 
            THEN coalesce(rag.initial_balance, 0) 
        WHEN acc.cuenta_tipo='A' OR ag.cuenta_tipo='A' 
            THEN coalesce(rag.initial_balance * -1, 0) 
        ELSE
            coalesce(rag.initial_balance, 0) 
    ...
    CASE 
        WHEN acc.cuenta_tipo='D' OR ag.cuenta_tipo='D' 
            THEN coalesce(rag.initial_balance, 0) + coalesce(rag.final_debit - rag.initial_debit, 0) - 
            coalesce(rag.final_credit - rag.initial_credit, 0)
        WHEN acc.cuenta_tipo='A' OR ag.cuenta_tipo='A'
            THEN coalesce(rag.initial_balance *-1, 0) + coalesce(rag.final_credit - rag.initial_credit, 0) - 
            coalesce(rag.final_debit - rag.initial_debit, 0)
        ELSE 
            coalesce(rag.final_balance, 0)
...
FROM
    ...
    LEFT JOIN account_group AS ag
        ON acc.group_id = ag.id
WHERE
    acc.id in %s
        """
