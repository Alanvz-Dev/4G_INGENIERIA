...
class IncidenciasNomina(models.Model):
    ...
    sueldo_liquidacion_4g = fields.Float('Sueldo Liquidación 4G')
    ...
    def _compute_sueldo(self):
        if self.sueldo_mensual:
            values = {
            'sueldo_diario': self.sueldo_mensual/30.4,
            'sueldo_por_horas': self.sueldo_mensual/30.4/8,
    ...
    def calculate_sueldo_cotizacion_base(self): 
        if self.employee_id.contract_ids[0].date_start: 
            ...
            sdi = ((365 + tablas_cfdi_line.aguinaldo + (tablas_cfdi_line.vacaciones)* (tablas_cfdi_line.prima_vac/100) ) / 365 ) * self.sueldo_mensual/30.4
            if sdi > max_sdi:
    ...
    @api.model 
    def calculate_sueldo_liquidacion_4g(self):
        tablas_cfdi = self.env['tablas.cfdi'].search([],limit=1)
        empleado = self.employee_id
        print(empleado)
        sueldo_diario=(self.sueldo_mensual/30.4)
        bono_a=sueldo_diario*self.employee_id.contract_ids.bono_asistencia_amount/100
        bono_p=sueldo_diario*self.employee_id.contract_ids.bono_puntualidad_amount/100
        vales=tablas_cfdi.uma*self.employee_id.contract_ids.vale_despensa_amount/100
        fondo_ahorro=sueldo_diario*0.075
        print(sueldo_diario+bono_a+bono_p+vales+fondo_ahorro)
        print(sueldo_diario,bono_a,bono_p,vales,fondo_ahorro)
        return sueldo_diario+bono_a+bono_p+vales+fondo_ahorro
