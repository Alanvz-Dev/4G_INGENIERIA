...
class PurchaseOrder(models.Model):
    ...
    approver_uid = fields.Many2one('res.partner',compute='_compute_approver_uid')
    def _compute_approver_uid(self):
        approver_uid =self.env['mail.message'].search([('model','in',['purchase.order']),('res_id','in',self.ids),('subtype_id','in',[14])], limit=1, order='id desc')
        print(approver_uid)
        self.approver_uid=approver_uid.author_id
...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_account_id(self):
        if not self.product_id:
            return
        self.product_id.write({'property_account_expense_id':self.account_id.id})
    ...
    def onchange_product_id(self):
        ...
        self.product_id.property_account_expense_id =self.account_id
