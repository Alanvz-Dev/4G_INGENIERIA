...
class HrPayslip(models.Model):
    ...
    def create(self, vals):
        x=self.env['hr.contract'].browse(int(vals['contract_id']))
        if 'journal_id' in self.env.context:
            vals['journal_id'] = self.env.context.get('journal_id')
        if not x.journal_id.id:
            return(_('El Empleado  "%s" no tiene un diario configurado! o no se ha relacionado un USUARIO con un EMPLEADO') % (str(x.display_name))+'\tEstado del contrato'+str(x.state))
        vals['journal_id'] = x.journal_id.id
